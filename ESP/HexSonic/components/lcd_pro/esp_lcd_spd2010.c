...
static esp_err_t panel_spd2010_sleep_on_off(esp_lcd_panel_t *panel, bool on_off);
...
spd2010_panel_t my_spd2010_pannel;
...
esp_err_t esp_lcd_new_panel_spd2010(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel) {
    ...
    spd2010->base.disp_sleep = panel_spd2010_sleep_on_off;
...
static const spd2010_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // 供应商给的30 fps的设置
    // {0x0A, (uint8_t []){0x22}, 1, 0},
    // {0x0B, (uint8_t []){0x28}, 1, 0},
    // {0x0C, (uint8_t []){0x28}, 1, 0},
    ...
    //===TP 设置，若不使用TP功能，以下几行代码取消=== ==
    //(0xFF 0x20 0x10 0x50}, 1, 0},
    //(0x08 0x55}, 1, 0},
    //(0x05 0x08}, 1, 0},
    //(0x01 0xA6}, 1, 0},
    //(0x00 0xA6}, 1, 0},
    ...
    // {0x44, (uint8_t []){0x00,0x20}, 2, 0},
    {0x35, (uint8_t[]){0x00}, 1, 0},

    {0x3A, (uint8_t[]){0x05}, 1, 0}, // RGB16位 ,0x07=RGB24位, 0x06=RGB18位
...
static esp_err_t panel_spd2010_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data) {
    ...
    esp_err_t err_rc;
    err_rc = tx_param(spd2010, io, LCD_CMD_CASET, (uint8_t[]){
                                                      (x_start >> 8) & 0xFF,
                                                      x_start & 0xFF,
                                                      ((x_end - 1) >> 8) & 0xFF,
                                                      (x_end - 1) & 0xFF,
                      ...
                      4);
    if (err_rc != ESP_OK) {
        ESP_LOGE(TAG, "send command failed1");
        return err_rc;
    }
    ...
    err_rc = tx_param(spd2010, io, LCD_CMD_RASET, (uint8_t[]){
                                                      (y_start >> 8) & 0xFF,
                                                      y_start & 0xFF,
                                                      ((y_end - 1) >> 8) & 0xFF,
                                                      (y_end - 1) & 0xFF,
                      ...
                      4);
    if (err_rc != ESP_OK) {
        ESP_LOGE(TAG, "send command failed2");
        return err_rc;
    }
    ...
    err_rc = tx_color(spd2010, io, LCD_CMD_RAMWR, color_data, len);
    if (err_rc != ESP_OK) {
        // ESP_LOGE(TAG,"send color failed,reboot this thread");
        return err_rc;
    }
    return ESP_OK;
}
...
static esp_err_t panel_spd2010_sleep_on_off(esp_lcd_panel_t *panel, bool on_off) {
    ...
    if (on_off) {
        command = LCD_CMD_IDMON;
    } else {
        command = LCD_CMD_IDMOFF;
    }
    ESP_RETURN_ON_ERROR(tx_param(spd2010, io, command, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}